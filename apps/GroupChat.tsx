...
const GroupChat: React.FC = () => {
    ...
    const handleGroupSummary = async () => {
        ...
        if (groupMembers.length === 0) {
            addToast('群聊中没有可归档成员', 'error');
            return;
        }

        const cleanArchiveOutput = (raw: string) => {
            return (raw || '')
                .replace(/```(?:markdown|yaml|json)?/gi, '')
                .replace(/```/g, '')
                .replace(/^\s*(summary|memory|archive)\s*:\s*/i, '')
                .trim();
        };
        ...
        try {
            ...
            for (let i = 0; i < dates.length; i++) {
                ...
                const logText = dayMsgs.map((m, idx) => {
                ...
                for (let memberIndex = 0; memberIndex < groupMembers.length; memberIndex++) {
                    ...
                    setSummaryProgress(`正在归档 ${date} (${i + 1}/${dates.length}) · ${member.name} (${memberIndex + 1}/${groupMembers.length})`);

                    const memberModel = member.apiModel?.trim() || apiConfig.model;

                    const prompt = `当前日期: ${date}
群聊名称: ${activeGroup.name}
当前要写入记忆的角色: ${member.name}
...
${logText.substring(0, 12000)}`;
                    ...
                    if (!response.ok) {
                        console.warn(`群聊归档失败: ${member.name}`, response.status);
                        continue;
                    }

                    const data = await safeResponseJson(response);
                    const rawContent = data.choices?.[0]?.message?.content?.trim() || '';
                    const summaryText = cleanArchiveOutput(rawContent);

                    if (!summaryText || summaryText === '无可归档内容') {
                        continue;
                    }
            ...
            addToast('群聊记忆已按成员模型同步完成', 'success');
            setModalType('none');
...
- 回复要保留自己的角色味。`;
            ...
            const charNameById = new Map(groupMembers.map(m => [m.id, m.name]));
            const formatGroupMemoryMessage = (msg: Message): string => {
                const speaker = msg.role === 'user'
                    ? (userProfile?.name || '用户')
                    : (msg.charId ? charNameById.get(msg.charId) : undefined) || '群友';
                const rawText = typeof msg.content === 'string' ? msg.content : '';
                const content = msg.type === 'image' ? '[图片]'
                    : msg.type === 'emoji' ? '[表情包]'
                    : msg.type === 'file' ? `[文件] ${msg.metadata?.name || rawText || ''}`
                    : msg.type === 'transfer' ? `[转账/红包] ${msg.metadata?.amount || ''}`
                    : /^(data:|https?:\/\/)/i.test(rawText.trim()) ? '[媒体]'
                    : rawText.trim();
                const replyPrefix = msg.replyTo
                    ? `引用 ${msg.replyTo.name}: ${String(msg.replyTo.content || '').slice(0, 80)} / `
                    : '';
                return `${speaker}: ${replyPrefix}${content}`.trim();
            };
            const groupMemoryMessages = currentMsgs
                .slice(-30)
                .filter(m => m.role === 'user' || (m.role === 'assistant' && !!m.charId))
                .filter(m => m.type === 'text' || m.type === 'file' || m.type === 'image' || m.type === 'emoji' || m.type === 'transfer');
            const groupMemoryQuery = [
                `群聊「${activeGroup.name}」的公开上下文。`,
                `当前模式：${GROUP_MODE_LABELS[groupMode]}。`,
                '只能把下面这些公开出现过的内容当作可见事实；未出现在这里的私聊、夹页、隐藏设定或其他角色记忆都不可当作已知。',
                groupMemoryMessages.slice(-18).map(formatGroupMemoryMessage).filter(Boolean).join('\n'),
            ].filter(Boolean).join('\n').slice(0, 2400);
            ...
            for (const member of groupMembers) {
                ...
                await injectMemoryPalace(
                    member,
                    groupMemoryMessages.length > 0 ? groupMemoryMessages : privateMsgs,
                    groupMemoryQuery,
                    userProfile?.name || '用户',
                );
